revoke all on function public.claim_target_practice_reward(integer, integer, integer) from public, anon, authenticated;
drop function public.claim_target_practice_reward(integer, integer, integer);
...
begin
  ...
  if p_round_id is null
    or p_duration_ms < 28000 or p_duration_ms > 45000
    or p_hits < 7 or p_hits > 120
    or p_score > p_hits * 1200 then
    raise exception 'This round could not be verified';
  end if;

  v_reference := 'target-practice-4:round:' || v_user_id::text || ':' || p_round_id::text;
  ...
  if exists (select 1 from public.coin_ledger where reference_key = v_reference) then
    ...
    return jsonb_build_object(
      ...
      'message', 'This round was already credited',
      'balance', v_balance
    );
  end if;
  ...
  values (
    ...
    v_reference,
    jsonb_build_object(
      'description', 'Target Practice 4 qualifying round',
      'round_id', p_round_id,
      'score', p_score,
      'hits', p_hits
    )
  ...
  return jsonb_build_object(
    'status', 'credited',
    'message', '5 Motion Coins added for this round',
    'balance', v_balance + 5
  );
exception
...
revoke all on function public.claim_target_practice_reward(integer, integer, integer, uuid) from public, anon;
grant execute on function public.claim_target_practice_reward(integer, integer, integer, uuid) to authenticated;
