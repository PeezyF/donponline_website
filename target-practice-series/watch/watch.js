(() => {
  ...
  async function init() {
    ...
    if (!episode) { state.innerHTML = '<h2>EPISODE NOT FOUND</h2><a class="button" href="../index.html">Return to Series</a>'; return; }
    ...
    if (!user || !await owns(episode)) { state.innerHTML = `<p class="kicker">PREMIUM EPISODE</p><h2>THIS EPISODE IS LOCKED</h2><p>${user ? "Return to the series to unlock it with Motion Coins." : "Sign in to unlock and watch Target Practice 4."}</p><a class="button primary" href="${user ? "../index.html#episodes" : "../../members.html"}">${user ? "View Unlock Options" : "Sign In"}</a>`; return; }
    const { data: signed, error } = await client.storage.from("target-practice-series").createSignedUrl(episode.video_path, 3600);
    if (error || !signed?.signedUrl) { state.innerHTML = '<p class="kicker">ACCESS CONFIRMED</p><h2>EPISODE UPLOAD IN PROGRESS</h2><p>Your purchase is safe. The 4K master is still being prepared for streaming.</p><a class="button" href="../index.html">Browse Episodes</a>'; return; }
